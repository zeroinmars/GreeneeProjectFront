...
const AddEvent = () => {
  ...
  const handleFormSubmit = () => {
    ...
    console.log({...eventInfo, checkWeeks, email, checkSpecial, preAlarm, tag, tag2, cateList,});
    if (email) {
      dispatch({type:"PROGRESS", progress:{progressToggle:true}});
    axios.post(url, {...eventInfo, checkWeeks, email, checkSpecial, preAlarm, tag, tag2, cateList})
    .then((res)=>{
